...
contract CakeFlipVaultTester is IStrategyLegacy, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    constructor(uint _pid) public {
        ...
        setMinter(IKoinMinter(0x0B4A714AAf59E46cb1900E3C031017Fd72667EfE));
    }
    ...
    // return cakeAmount, koinAmount, 0
    function profitOf(address account) override public view returns (uint _usd, uint _koin, uint _bnb) {
        ...
        if (address(minter) != address(0) && minter.isMinter(address(this))) {
            ...
            // koin amount
            _koin = minter.amountKoinToMint(bnbValue);
        } else {
            _usd = amount;
            _koin = 0;
        }
    ...
    function setMinter(IKoinMinter _minter) public onlyOwner {
        // can zero
        minter = _minter;
