...
abstract contract PancakeSwap {
    ...
    address private constant _koin = 0xC9849E6fdB743d08fAeE3E34dd2D1bc69EA11a51;
    address private constant _wbnb = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

    function koinBNBFlipToken() internal view returns(address) {
        return factory.getPair(_koin, _wbnb);
    }

    function tokenToKoinBNB(address token, uint amount) internal returns(uint flipAmount) {
        if (token == cake) {
            flipAmount = _cakeToKoinBNBFlip(amount);
        } else {
            // flip
            flipAmount = _flipToKoinBNBFlip(token, amount);
        }
    }

    function _cakeToKoinBNBFlip(uint amount) private returns(uint flipAmount) {
        swapToken(cake, amount.div(2), _koin);
        swapToken(cake, amount.sub(amount.div(2)), _wbnb);

        flipAmount = generateFlipToken();
    }

    function _flipToKoinBNBFlip(address token, uint amount) private returns(uint flipAmount) {
        ...
        if (_token0 == _wbnb) {
            swapToken(_token1, IBEP20(_token1).balanceOf(address(this)), _koin);
            flipAmount = generateFlipToken();
        } else if (_token1 == _wbnb) {
            swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), _koin);
            flipAmount = generateFlipToken();
        } else {
            swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), _koin);
    ...
    function generateFlipToken() private returns(uint liquidity) {
        uint amountADesired = IBEP20(_koin).balanceOf(address(this));
        uint amountBDesired = IBEP20(_wbnb).balanceOf(address(this));

        IBEP20(_koin).safeApprove(address(ROUTER), 0);
        IBEP20(_koin).safeApprove(address(ROUTER), amountADesired);
        IBEP20(_wbnb).safeApprove(address(ROUTER), 0);
        IBEP20(_wbnb).safeApprove(address(ROUTER), amountBDesired);

        (,,liquidity) = ROUTER.addLiquidity(_koin, _wbnb, amountADesired, amountBDesired, 0, 0, address(this), block.timestamp);

        // send dust
        IBEP20(_koin).transfer(msg.sender, IBEP20(_koin).balanceOf(address(this)));
